...
{
	if ((self=[super initWithFrame:CGRectMake(rect.origin.x,rect.origin.y,rect.size.width,27)]))
	{
...
-(void)awakeFromNib
{
	...
	[self setThumbImage:[UIImage imageNamed:@"Playback_Order_green"] forState:UIControlStateNormal];
	[self setMinimumTrackImage:[UIImage imageNamed:@"imageSwitchGrayBG"] forState:UIControlStateNormal];
	[self setMaximumTrackImage:[UIImage imageNamed:@"imageSwitchGrayBG"] forState:UIControlStateNormal];
	...
	self.clippingView = [[UIView alloc] initWithFrame:CGRectMake(4,2,216,23)];
...
//	[self.clippingView release];
	...
	self.leftLabel.font = [UIFont boldSystemFontOfSize:FONT_SIZE];
...
//	[self.leftLabel release];
	...
	self.rightLabel.font = [UIFont boldSystemFontOfSize:FONT_SIZE];
...
//    NSLog(@"thumbWidth=%f    self.bounds.size.width=%f\n\n",thumbWidth,self.bounds.size.width);
//    NSLog(NSStringFromCGRect(self.rightLabel.frame));
}
...
- (void)setOn:(BOOL)turnOn animated:(BOOL)animated;
{
	...
	{
		self.value = 0.0;
        [self setThumbImage:[UIImage imageNamed:@"Play_All_Once_green"] forState:UIControlStateNormal];
